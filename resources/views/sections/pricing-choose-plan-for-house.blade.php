@php
    ...
    $title = $data['title'] ?? 'Choose your executive workspace';
    $backgroundImage = $data['background_image'] ?? 'https://xinterio-demo.pbminfotech.com/html-demo/images/homepage-1/bg/pricing-bg-shape.png';
    $benefits = $data['benefits'] ?? [];
    $plans = $data['plans'] ?? [];
@endphp
			<section class="section-xl pricing-one-bg" style="background-image: url('{{ image($backgroundImage, 'section_background') }}')">
				<div class="container">
					<div class="row g-0">
...
@foreach($data['features'] ?? [] as $benefit)
